...
class GoogLeNet(nn.Module):
    ...
    # num_classed:需要分类的类别数量
    # aux_logits:训练过程是否需要使用复制分类器
    ...
    def forward(self, x):
        ...
        # N x 512 x 14 x 14
        # 设置.train()为训练模式时，self.training=True
        ...
        for m in self.modules():
            ...
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.kaiming_normal_(m.weight,
                                        mode='fan_out',
                                        nonlinearity='relu')
                # 如果偏置不是0，将偏置置为0，对偏置进行初始化
                if m.bias is not None:
                    # torch.nn.init.constant_(tensor, val)，初始化整个矩阵为常量val
                    nn.init.constant_(m.bias, 0)

            # 如果是全连接层
            elif isinstance(m, nn.Linear):
                # init.normal_(tensor, mean=0.0, std=1.0)，使用从正态分布中提取的值填充入张量
...
class InceptionAux(nn.Module):
    ...
    def __init__(self, in_channels, num_classes) -> None:
        ...
        # 上一层output[batch, 128, 4, 4], 128 x 4 x 4 = 2048
        self.fc1 = nn.Linear(2048, 1024)
        self.fc2 = nn.Linear(1024, num_classes)

    # 向前传播过程
    def forward(self, x):
        # 输入：分类器1：N x 512 x 14 x 14，分类器2：N x 528 x 14 x 14
